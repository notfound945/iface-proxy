...
async fn handle_socks5(mut inbound: TcpStream, iface: &str, user: Option<&str>, pass: Option<&str>) -> Result<()> {
    ...
    match cmd {
        0x01 => {
            log_throttled(|| println!("SOCKS5 CONNECT -> {}:{} (iface: {})", target_host, target_port, iface));
            ...
            log_throttled(|| println!("SOCKS5 finished {}:{} (c->s: {} bytes, s->c: {} bytes)", target_host, target_port, c2s, s2c));
            Ok(())
        }
...
pub async fn run_socks5_proxy_auth(iface: &str, listen: &str, user: Option<&str>, pass: Option<&str>) -> Result<()> {
    ...
    loop {
        ...
        log_throttled(|| println!("Incoming TCP connection from {} -> listening on {} (iface: {})", peer_addr, listen_for_log, iface));
